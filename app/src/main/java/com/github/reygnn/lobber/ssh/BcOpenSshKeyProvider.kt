 ...
 *
 * Hintergrund: sshj's [net.schmizz.sshj.common.Ed25519KeyFactory] baut
 * PKCS#8-Bytes mit hardcoded Preamble und ruft `KeyFactory.getInstance("Ed25519")`.
 * Auf Android landet das oft auf Conscrypt — dem System-Provider, der die
 * Preamble nicht akzeptiert ("invalid EdDSA PKCS#8 key preamble"). Auch ein
 * vorab registriertes BouncyCastle hilft nicht zuverlässig: sshj's interne
 * `SecurityUtils.register()` hat Edge-Cases, in denen der Provider-Name
 * ungesetzt bleibt und der Default-JCE-Lookup wieder Conscrypt erreicht.
 *
 * Hier umgehen wir den Pfad in zwei Schritten:
 *  - [KeyFactory.getInstance] wird mit einer **konkreten** BouncyCastle-Instanz
 *    aufgerufen, nicht über den Provider-Namen → garantiert BC.
 *  - [OpenSSHPrivateKeySpec] / [OpenSSHPublicKeySpec] → BC parst den OpenSSH-
 *    Blob direkt ohne PKCS#8-Umweg, kein Preamble-Mismatch möglich.
 */
