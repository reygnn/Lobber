...
data class SshKeyPair(
    /** OpenSSH-PEM-Block, kompatibel zu sshj's `loadKeys(String, …)`. */
...
object SshKeygen {
    ...
    fun generateEd25519(comment: String = "lobber@android"): SshKeyPair {
    ...
    private fun wrapPem(blob: ByteArray): String = buildString {
        ...
        Base64.toBase64String(blob).chunked(70).forEach {
            append(it)
            append('\n')
        }
